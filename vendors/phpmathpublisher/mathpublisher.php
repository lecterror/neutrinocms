...
define('DIR_FONTS', realpath(dirname(__FILE__).DS.'fonts'));
// $dirfonts = realpath(dirname(__FILE__).DS.'fonts');

// absolute path to the images directory
// $dirimg=$_SERVER["DOCUMENT_ROOT"]."/phpmathpublisher/img";
// $dirimg = APP.WEBROOT_DIR.DS.'img'.DS.'m'; // (dirname(__FILE__).DS.'..'.DS.'webroot'.DS.'img'.DS.'m');
define('DIR_IMG', APP.WEBROOT_DIR.DS.'img'.DS.'m');

// die($dirfonts.'<br />'.$dirimg);
...
switch($texte)
	...
	break;
	case 'right':
	$font =DIR_FONTS."/".$fontesmath[$texte].".ttf";
	$t=16;
	...
	case '_dintegrale':
	case '_tintegrale':
	if(isset($fontesmath[$texte])) $font = DIR_FONTS."/".$fontesmath[$texte].".ttf";
	elseif (est_nombre($texte)) $font = DIR_FONTS."/cmr10.ttf";
	else $font = DIR_FONTS."/cmmi10.ttf";
	$t=6;
...
$texte=stripslashes($texte);
if(isset($fontesmath[$texte])) $font = DIR_FONTS."/".$fontesmath[$texte].".ttf";
elseif (ereg("[a-zA-Z]", $texte)) $font = DIR_FONTS."/cmmi10.ttf";
else $font = DIR_FONTS."/cmr10.ttf";
if(isset($symboles[$texte])) $texte = $symboles[$texte];
$htexte = 'dg'.$texte;
$hdim = ImageTTFBBox($taille, 0, $font, $htexte);
$wdim = ImageTTFBBox($taille, 0, $font, $texte);
